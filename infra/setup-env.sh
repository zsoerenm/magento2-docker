...
resolve_config() {
  SETUP_CONFIG="$CONFIG" SETUP_ENV="$ENV" python3 <<'PYEOF'
import tomllib, os, re, sys

with open(os.environ["SETUP_CONFIG"], "rb") as f:
...
print(f'BACKEND_FRONTNAME={get("admin", "admin_url_path")}')
...
print(f'TRANS_EMAIL_NAME={get("email", "sender_name")}')
print(f'TRANS_EMAIL_ADDRESS={get("email", "sender_address")}')
print(f'SMTP_TRANSPORT=smtp')
...
print(f'SMTP_AUTH={get("email", "smtp_authentication")}')
print(f'SMTP_SSL={get("email", "smtp_encryption")}')
...
chmod 644 "$DEPLOY_DIR/docker/secrets/"*.txt
...
if [ "$CADDY_CONFIG" = "staging" ]; then
  ...
  if [ -n "$HASH" ]; then
    ...
    # Write credentials directly into Caddyfile (avoids Docker Compose $ expansion issues with bcrypt hashes)
    STAGING_USER=$(get_val STAGING_USERNAME)
    sed -i "s|{\$STAGING_USERNAME}|$STAGING_USER|g; s|{\$STAGING_PASSWORD_HASH}|$HASH|g" "$CADDY_CONF_DIR/Caddyfile"
